...
CREATE TABLE T_ALERT (
    ...
    BOARD_TYPE VARCHAR(20) NOT NULL COMMENT '글 종류',
    ALERT_TYPE VARCHAR(20) NOT NULL COMMENT '알림 종류 (예: "게시글 작성", "요청 작성")',
    ALERT_OBJECT_ID BIGINT COMMENT '알림과 연결된 객체 ID (예: 요청 ID, 게시글 ID)',
    CONTENT VARCHAR(200) NOT NULL COMMENT '알림 내용',
    SENDER_ID VARCHAR(50) COMMENT '알림 발송인',
...
-- T_ALERT(복합인덱스 추가)
CREATE INDEX idx_receiver_id_read_yn ON T_ALERT (RECEIVER_ID, READ_YN);
...
-- T_TICKET_INFO  요청(요청사항) 정보
-- DROP TABLE T_TICKET_INFO;
-- 2024/12/10 N회차 변경

CREATE TABLE `T_TICKET_INFO` (
  `TICKET_ID` int(11) NOT NULL AUTO_INCREMENT COMMENT '요청번호',
  ...
  `PARENT_TICKET_CD` int(11) DEFAULT NULL COMMENT '연관요청',
...
CREATE TABLE `T_TICKET_ANSWER` (
  ...
  `TICKET_ID` int(11) NOT NULL COMMENT '요청번호',
  `STATUS_CD` varchar(20) NOT NULL COMMENT '상태코드',
  `CONTENT` longtext NOT NULL COMMENT '내용',
  `CREATE_ID` varchar(50) NOT NULL COMMENT '작성자아이디',
  `CREATE_DT` datetime NOT NULL COMMENT '작성일',
