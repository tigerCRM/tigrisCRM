 ...
 */
public class MD4Utils extends MessageDigest implements Cloneable {
	...
	public static String md4key(String inputString) throws UnsupportedEncodingException {
        MessageDigest digester = new MD4Utils();
        byte[] md4bytes = digester.digest(inputString.getBytes("UnicodeLittleUnmarked")); // UnicodeLittleUnmarked: Sixteen-bit Unicode Transformation Format, little-endian byte order
        char[] encoded = Hex.encodeHex(md4bytes);
        String genPwd = new String(encoded);
        return genPwd;
	}
}
